...
func getPublicRandomness(c *cli.Context, lg log.Logger) error {
	...
	info := chain.NewChainInfo(lg, group)

	var resp client.Result
	var foundCorrect bool
	for _, id := range ids {
		grpcClient, err := grpc.New(lg, id.Addr, certPath, !id.TLS, info.Hash())
		if err != nil {
...
func getChainInfo(c *cli.Context, lg log.Logger) error {
	...
	grpcClient := core.NewGrpcClient(lg, chainHash)
	if c.IsSet(tlsCertFlag.Name) {
		defaultManager := net.NewCertManager(lg)
		certPath := c.String(tlsCertFlag.Name)
		if err := defaultManager.Add(certPath); err != nil {
			return err
		}
		grpcClient = core.NewGrpcClientFromCert(lg, chainHash, defaultManager)
	}
	var ci *chain.Info
